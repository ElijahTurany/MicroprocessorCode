...
circular_buffer * _tx_buffer1;                                    //missing code here								  //missing code here
...
int buffer_peek(portType uart)
{
	...
	{
		/*fill in the implementation when needed(see debug_port code below)*/
...
int buffer_read(portType uart)
{
	if (uart == bluetooth_port)
	{
	  /*fill in the implementation (see debug_port code below)*/
		if(_rx_buffer1->head == _rx_buffer1->tail)
						{
						...
						{   /*read new data at tail position into a variable called'c'*/
							unsigned c = 0;                    //error here
		                    /*compute new tail position*/
							_rx_buffer1->tail = (uint32_t)UART_BUFFER_SIZE;  //error here
							return c;
						}
	}
		...
		if(_rx_buffer2->head == _rx_buffer2->tail)
				...
				{   /*read new data at tail position into a variable called'c'*/
					unsigned c = 0;                    //error here
                    /*compute new tail position*/
					_rx_buffer2->tail = (uint32_t)UART_BUFFER_SIZE;  //error here
					return c;
				}
		}
...
void buffer_write(unsigned char c, portType uart)
{
	if (uart == bluetooth_port)
	{
		/*fill in the implementation (see debug_port code below when needed)*/
		int loc =(uint32_t) UART_BUFFER_SIZE;  //error here with a wrong write buffer position
